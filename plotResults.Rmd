...
studyAreaLocation <- file.path(Paths$inputPath, "studyArea/PiecewiseSmoothing/studyArea_AB_BCR6", .studyAreaName)
...
forClassRaster <- terra::classify(forClassRaster, reclassMatrixFcr)
...
landClassRaster <- terra::classify(landClassRaster, reclassMatrixLcr)
...
classPalLc <- c(
  "#3C6987FF", "#87A5A5FF", "#4B3C4BFF", "#2D3C5AFF",
  "#D2D2C3FF", "#A5A587FF")
classPalFc <- c( "#B4783CFF", "#D2A54BFF",
  "#B4B45AFF", "#78963CFF", "#1E4B3CFF", "#5A695AFF"
)

cbPalette2 <- c("#56B4E9", "#0072B2", "#009E73", "#E69F00", "#999999", "black")
#cbPalette1 <- c( "#3C6987FF", "#87A5A5FF", "#4B3C4BFF", "#2D3C5AFF",
#  "#D2D2C3FF", "#A5A587FF")
#cbPalette2 <- c( "#B4783CFF", "#D2A54BFF",
#  "#B4B45AFF", "#78963CFF", "#1E4B3CFF", "#5A695AFF")



op <- par(cex = 2) # change legend text size
plot(forClassRaster, main = "Forest Class Raster", legend = TRUE, col = classPalFc, colNA = "white", axes = FALSE, box = FALSE)
# legend("bottomright", legend = c( "Black Spruce", "Black Spruce Wet", "Conifer Mix", "Deciduous", "Mixed", "Pine", "White Spruce"), fill = cbPalette)
plot(forClassRaster, legend = TRUE, col = classPalFc, colNA = "white", axes = FALSE, box = FALSE)
...
plot(landClassRaster, main = "Land-Cover Class Raster", legend = TRUE, col = classPalLc, colNA = "white", axes = FALSE, box = FALSE)

plot(landClassRaster, legend = TRUE, col = classPalLc, colNA = "white", axes = FALSE, box = FALSE)

ageRaster <- terra::project(ageRaster, y = "+proj=longlat +datum=WGS84")
ageRaster <- crop(ageRaster, studyArea)

pal <- colorRampPalette(c("#D2D2C3FF","#A5A587FF","#78963CFF", "#1E4B3CFF", "black"))


plot(ageRaster, main = "Age Raster", colNA = "white", axes = FALSE, box = FALSE, col = pal(75)) 

plot(ageRaster, colNA = "white", axes = FALSE, box = FALSE, col = pal(75))
```
...
sp.labs <- c(
  "Alder Flycatcher", "Bay-breasted Warbler", "Black-capped Chickadee",
  "Boreal Chickadee", "Brown Creeper", "Canada Jay", "Cape May Warbler",
  "Common Yellowthroat", "Dark-eyed Junco", "Golden-crowned Kinglet",
  "Least Flycatcher", "Mourning Warbler",
  "Ovenbird", "Palm Warbler", "Red-breasted Nuthatch",
  "Ruby-crowned Kinglet", "Red-eyed Vireo", "Swainson’s Thrush",
  "Tennessee Warbler", "Yellow-rumped Warbler"
)
...
lapply(spList, FUN = function(sp) {
  ...
  ggsave(file.path(paste(saveLocation, "/", sp, "_rasterPlots.jpeg", sep = "")), rasterPlot, width = 22, height = 35, units = "cm")
})
```
